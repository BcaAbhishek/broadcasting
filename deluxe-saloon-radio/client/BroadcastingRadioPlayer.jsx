...
export default function BroadcastingRadioPlayer() {
  ...
  const loadingLabel = slowStart ? "Waking up the radio server… (can take up to a minute)" : "Loading…";
  ...
  return (
    <div className="dsr-root">
      ...
      <style>{`
        ...
        .dsr-badge-row { display: flex; align-items: center; gap: 8px; }
        .dsr-badge {
          display: flex; align-items: center; gap: 6px;
          padding: clamp(5px, 1.2vw, 6px) clamp(10px, 2.5vw, 13px);
          border-radius: 999px; background: rgba(15,8,3,0.42); backdrop-filter: blur(6px);
          color: rgba(255,246,232,0.92); font-size: clamp(10.5px, 1.4vw, 14px); font-weight: 500;
          white-space: nowrap; margin-left: auto;
        }
        .dsr-share-btn {
          width: clamp(26px, 3.4vw, 32px); height: clamp(26px, 3.4vw, 32px); border-radius: 50%;
          border: none; background: rgba(15,8,3,0.42); backdrop-filter: blur(6px);
          display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0;
        }
        .dsr-offline {
          display: flex; align-items: center; gap: 8px;
          padding: clamp(8px, 1.4vw, 13px) clamp(10px, 1.6vw, 16px); border-radius: 12px;
          background: rgba(58,20,14,0.75); backdrop-filter: blur(6px);
          border: 1px solid rgba(220,110,90,0.4); color: #fbf3e6; font-size: clamp(10.5px, 1.4vw, 14px);
        }
        .dsr-special {
          display: flex; align-items: center; justify-content: center; gap: 8px;
          padding: clamp(8px, 1.4vw, 13px) clamp(10px, 1.6vw, 16px); border-radius: 12px;
          background: rgba(28,16,8,0.72); backdrop-filter: blur(6px);
          border: 1px solid rgba(255,255,255,0.14);
          border-top: 4px solid;
          border-image: linear-gradient(90deg, #ff9933 33%, #ffffff 33%, #ffffff 66%, #138808 66%) 1;
          color: #fbf3e6; font-size: clamp(11px, 1.6vw, 15px); font-weight: 600; text-align: center;
        }
        ...
        .dsr-nextup {
          display: flex; align-items: center; gap: 6px; align-self: flex-start; max-width: 100%;
          padding: 4px clamp(9px, 1.6vw, 12px); border-radius: 999px;
          background: rgba(15,8,3,0.42); backdrop-filter: blur(6px);
          color: rgba(251,243,230,0.75); font-size: clamp(9.5px, 1.1vw, 12px); white-space: nowrap; overflow: hidden;
        }
        .dsr-nextup span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .dsr-album {
          width: clamp(42px, 6vw, 72px); height: clamp(42px, 6vw, 72px); border-radius: 50%;
        ...
        .dsr-icon-btn {
          width: clamp(30px, 4.5vw, 42px); height: clamp(30px, 4.5vw, 42px); min-width: 30px; min-height: 30px;
          border-radius: 50%; border: none; background: rgba(255,255,255,0.12);
          display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0;
        }
        .dsr-join-circle {
          width: clamp(52px, 8vw, 92px); height: clamp(52px, 8vw, 92px); border-radius: 50%; background: #fbf3e6;
      ...
      <div className="dsr-top-stack">
        <div className="dsr-badge-row">
          <div className="dsr-badge">
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: connected ? "#7ed858" : "#8a5a5a", animation: connected ? "pulseGlow 2s ease-out infinite" : "none", flexShrink: 0 }} />
      ...
      {!joined && track && (
        <div
          ...
          style={{
            ...
            background: "rgba(10,6,2,0.55)", cursor: "pointer",
          }}
        >
          <div className="dsr-join-circle">
            <div style={{ width: 0, height: 0, marginLeft: 5, borderTop: "12px solid transparent", borderBottom: "12px solid transparent", borderLeft: "18px solid #6b3f16" }} />
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 7 }}>
              <div style={{ flex: 1, height: 4, borderRadius: 999, background: "rgba(255,255,255,0.16)", position: "relative" }}>
                <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${progressPct}%`, borderRadius: 999, background: "#e8a13a", transition: "width 0.9s linear" }} />
              </div>
...
function AlbumArt({ cover, spinning }) {
  return (
    <div
      className="dsr-album"
      style={{
        background: cover ? `url(${cover}) center/cover, #1a0e08` : "radial-gradient(circle, #d9a441 0%, #d9a441 22%, #1a0e08 23%, #1a0e08 100%)",
        animation: spinning ? "spin 4.5s linear infinite" : "none",
      }}
    >
      <div style={{ width: "27%", height: "27%", borderRadius: "50%", background: "rgba(20,10,6,0.55)", border: "2px solid rgba(255,255,255,0.25)" }} />
    </div>
  );
}
...
function Scene({ dim }) {
  return (
    <svg
      ...
      style={{ position: "absolute", inset: 0, display: "block", filter: dim ? "brightness(0.55) saturate(0.7)" : "brightness(1) saturate(1)", transition: "filter 0.6s ease" }}
    >
      ...
      <text x="360" y="97" textAnchor="middle" fontFamily="'Bebas Neue', sans-serif" fontSize="58" letterSpacing="4" fill="#fbf3e6">
        BROADCASTING RADIO
      </text>
